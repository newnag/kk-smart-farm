...
if($inputType === "News"){
	try {
		...
		$sql =" SELECT * FROM ".TABLE_MOD_ARTICLE." WHERE ".TABLE_MOD_ARTICLE."_Status='Enable' AND ".TABLE_MOD_ARTICLE."_Type = 'News'";
...
elseif($inputType === "BodyCon"){
	try {
		$counter=0; $arSQLData=array();
		$sql =" SELECT * FROM ".TABLE_MOD_ARTICLE." WHERE ".TABLE_MOD_ARTICLE."_Status='Enable' AND ".TABLE_MOD_ARTICLE."_Type = 'BodyCondition'";
		$sql.=" ORDER BY ".TABLE_MOD_ARTICLE."_".$SendRequest["inputShowOrderBy"]." ".$SendRequest["inputShowASCDESC"];
		$Query=$System_Connection->prepare($sql);
		if(sizeof($arSQLData)>0) { $Query->execute($arSQLData);  } else { $Query->execute(); }
		while($Row=$Query->fetch(PDO::FETCH_ASSOC)) {
...
elseif($inputType === "Nutrition"){
	try {
		$counter=0; $arSQLData=array();
		$sql =" SELECT * FROM ".TABLE_MOD_ARTICLE." WHERE ".TABLE_MOD_ARTICLE."_Status='Enable' AND ".TABLE_MOD_ARTICLE."_Type = 'Nutrition'";
		$sql.=" ORDER BY ".TABLE_MOD_ARTICLE."_".$SendRequest["inputShowOrderBy"]." ".$SendRequest["inputShowASCDESC"];
		$Query=$System_Connection->prepare($sql);
		if(sizeof($arSQLData)>0) { $Query->execute($arSQLData);  } else { $Query->execute(); }
		while($Row=$Query->fetch(PDO::FETCH_ASSOC)) {
