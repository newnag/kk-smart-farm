...
try {
	$arSQLData=array();
	$sql =" SELECT COUNT(*) AS Counter FROM ".TABLE_MOD_BANNERHERO." WHERE ".TABLE_MOD_BANNERHERO."_Status='Enable' ";
	$Query=$System_Connection->prepare($sql);
	if(sizeof($arSQLData)>0) { $Query->execute($arSQLData);  } else { $Query->execute(); }
	$Rows=$Query->fetchAll();
...
try {
	...
	$sql =" SELECT * FROM ".TABLE_MOD_BANNERHERO." WHERE ".TABLE_MOD_BANNERHERO."_Status='Enable' ";
	$sql.=" ORDER BY ".TABLE_MOD_BANNERHERO."_".$SendRequest["inputShowOrderBy"]." ".$SendRequest["inputShowASCDESC"];
	$Query=$System_Connection->prepare($sql);
	if(sizeof($arSQLData)>0) { $Query->execute($arSQLData);  } else { $Query->execute(); }
	while($Row=$Query->fetch(PDO::FETCH_ASSOC)) {
		$DataField=array();
		$DataField["ID"]=$Row[TABLE_MOD_BANNERHERO."_ID"];
		$DataField["Name"]=$Row[TABLE_MOD_BANNERHERO."_Name"];
		if($Row[TABLE_MOD_BANNERHERO."_Picture"]<>"") {
			$DataField["Picture"]=SYSTEM_FULLPATH_UPLOAD."mod_bannerhero/".$Row[TABLE_MOD_BANNERHERO."_Picture"];
			$DataField["Picture-Thumb"]=SYSTEM_FULLPATH_UPLOAD."mod_bannerhero/thumb-".$Row[TABLE_MOD_BANNERHERO."_Picture"];
		} else {
			$DataField["Picture"]=CONFIG_DEFAULT_THUMB_PICTURE;
			$DataField["Picture-Thumb"]=CONFIG_DEFAULT_THUMB_PICTURE;
		}
		$DataField["CreateDate"]=$Row[TABLE_MOD_BANNERHERO."_CreateDate"];
		$DataField["CreateByID"]=$Row[TABLE_MOD_BANNERHERO."_CreateByID"];
		$DataField["CreateByName"]=$arUserNameByID[$Row[TABLE_MOD_BANNERHERO."_CreateByID"]];
		$DataField["LastUpdate"]=$Row[TABLE_MOD_BANNERHERO."_LastUpdate"];
		$DataField["LastUpdateByID"]=$Row[TABLE_MOD_BANNERHERO."_LastUpdateByID"];
		$DataField["LastUpdateByName"]=$arUserNameByID[$Row[TABLE_MOD_BANNERHERO."_LastUpdateByID"]];
		$DataField["Order"]=$Row[TABLE_MOD_BANNERHERO."_Order"];
		$DataField["Status"]=$Row[TABLE_MOD_BANNERHERO."_Status"];
		$DataRow[]=$DataField;
		$counter++;
	}
