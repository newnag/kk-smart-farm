...
try {
	$arSQLData=array();
	$sql =" SELECT COUNT(*) AS Counter FROM ".TABLE_MOD_PROMOTION." WHERE ".TABLE_MOD_PROMOTION."_Status='Enable' ";
	$Query=$System_Connection->prepare($sql);
	if(sizeof($arSQLData)>0) { $Query->execute($arSQLData);  } else { $Query->execute(); }
	$Rows=$Query->fetchAll();
...
try {
	...
	$sql =" SELECT * FROM ".TABLE_MOD_PROMOTION." WHERE ".TABLE_MOD_PROMOTION."_Status='Enable' ";
	$sql.=" ORDER BY ".TABLE_MOD_PROMOTION."_".$SendRequest["inputShowOrderBy"]." ".$SendRequest["inputShowASCDESC"];
	$Query=$System_Connection->prepare($sql);
	if(sizeof($arSQLData)>0) { $Query->execute($arSQLData);  } else { $Query->execute(); }
	while($Row=$Query->fetch(PDO::FETCH_ASSOC)) {
		$DataField=array();
		...
		$DataField["CreateDate"]=$Row[TABLE_MOD_PROMOTION."_CreateDate"];
		$DataField["CreateByID"]=$Row[TABLE_MOD_PROMOTION."_CreateByID"];
		$DataField["CreateByName"]=$arUserNameByID[$Row[TABLE_MOD_PROMOTION."_CreateByID"]];
		$DataField["LastUpdate"]=$Row[TABLE_MOD_PROMOTION."_LastUpdate"];
		$DataField["LastUpdateByID"]=$Row[TABLE_MOD_PROMOTION."_LastUpdateByID"];
		$DataField["LastUpdateByName"]=$arUserNameByID[$Row[TABLE_MOD_PROMOTION."_LastUpdateByID"]];
		$DataField["Order"]=$Row[TABLE_MOD_PROMOTION."_Order"];
