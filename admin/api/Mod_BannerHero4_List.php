...
try {
	$arSQLData=array();
	$sql =" SELECT COUNT(*) AS Counter FROM ".TABLE_MOD_BANNERHERO4." WHERE ".TABLE_MOD_BANNERHERO4."_Status='Enable' ";
	$Query=$System_Connection->prepare($sql);
	if(sizeof($arSQLData)>0) { $Query->execute($arSQLData);  } else { $Query->execute(); }
	$Rows=$Query->fetchAll();
...
try {
	...
	$sql =" SELECT * FROM ".TABLE_MOD_BANNERHERO4." WHERE ".TABLE_MOD_BANNERHERO4."_Status='Enable' ";
	$sql.=" ORDER BY ".TABLE_MOD_BANNERHERO4."_".$SendRequest["inputShowOrderBy"]." ".$SendRequest["inputShowASCDESC"];
	$Query=$System_Connection->prepare($sql);
	if(sizeof($arSQLData)>0) { $Query->execute($arSQLData);  } else { $Query->execute(); }
	while($Row=$Query->fetch(PDO::FETCH_ASSOC)) {
		$DataField=array();
		...
		$DataField["CreateDate"]=$Row[TABLE_MOD_BANNERHERO4."_CreateDate"];
		$DataField["CreateByID"]=$Row[TABLE_MOD_BANNERHERO4."_CreateByID"];
		$DataField["CreateByName"]=$arUserNameByID[$Row[TABLE_MOD_BANNERHERO4."_CreateByID"]];
		$DataField["LastUpdate"]=$Row[TABLE_MOD_BANNERHERO4."_LastUpdate"];
		$DataField["LastUpdateByID"]=$Row[TABLE_MOD_BANNERHERO4."_LastUpdateByID"];
		$DataField["LastUpdateByName"]=$arUserNameByID[$Row[TABLE_MOD_BANNERHERO4."_LastUpdateByID"]];
		$DataField["Order"]=$Row[TABLE_MOD_BANNERHERO4."_Order"];
